<template>
    ...
    <div class="home">
        ...
        <Modal ref="modalCancel" v-on:ok="onCancel">Отменить?</Modal>
        <Modal ref="modalDelete" v-on:ok="onDelete">Удалить?</Modal>
    </div>
</template>
...
export default {
    ...
    computed: {
        note() {
            return this.$store.state.notes[this.$route.params.index]
        },
    },
    created() {
        this.writeHistory()
    ...
    methods: {
        ...
        //Удалить заметку
        onDelete() {
            this.$store.commit('deleteNote', this.$route.params.index)
            this.$store.commit('save')
            this.$router.push('/')
        },
        ...
        // Добавить дело
        addItem() {
            this.$store.commit('addTodoItem', this.$route.params.index)
            this.writeHistory()
            this.historyCancel.splice(0)
        },
        // Удалить дело
